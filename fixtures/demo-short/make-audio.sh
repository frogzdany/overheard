#!/bin/bash
# make-audio.sh — synthesize demo-short.{mp3,wav} from meeting-transcript.jsonl
# using the ElevenLabs CLI (text-to-dialogue), exactly the same method as
# ../make-demo-audio.sh, but scoped to this directory's short meeting clip.
#
# Calls the ElevenLabs API and therefore costs credits and requires
...
# added, no audio tags — because expected-actions.json quotes it exactly.
#
# Usage:
#   ./make-audio.sh
#   ANA_VOICE_ID="..." DANIEL_VOICE_ID="..." ./make-audio.sh   # override voices
#
# Outputs (both overwritten idempotently on every run):
#   demo-short.mp3 — full quality (mp3_44100_128).
#   demo-short.wav — 16 kHz mono 16-bit PCM.
...
OUT_MP3="demo-short.mp3"
OUT_WAV="demo-short.wav"

CLI=(npx --yes @elevenlabs/cli@latest)

# Same voices as the long clip (fixtures/make-demo-audio.sh):
...
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/overheard-demo-short-audio.XXXXXX")"
trap 'rm -rf "$WORKDIR"' EXIT

# 1) Split the transcript lines into consecutive chunks that stay under
